...
def test_case_1():
    ...
    cleanup()
    generate_files()
    subprocess.run(["script.bat", LOG_DIR, BACKUP_DIR, "90", "5"], check=True)
...
def test_case_2():
    ...
    cleanup()
    generate_files()
    subprocess.run(["script.bat", LOG_DIR, BACKUP_DIR, "30", "20"], check=True)
...
def test_case_3():
    ...
    cleanup()
    generate_files()
    subprocess.run(["script.bat", LOG_DIR, BACKUP_DIR, "30", "12"], check=True)
    assert (len(os.listdir(BACKUP_DIR)) == 1) and (len(os.listdir(LOG_DIR)) == 8)  # проверка - архив в BACKUP_DIR и осталось 8 в LOG_DIR

def test_case_4():
    print("тест 4")
    cleanup()
    subprocess.run(["script.bat", LOG_DIR, BACKUP_DIR, "30", "5"], check=True)
    assert len(os.listdir(BACKUP_DIR)) == 0  # проверка - нет файлов в BACKUP_DIR
